...
std::optional<Lib> Lib::constructOptionally(const BasicValue &config,
                                            const BasicValue &lib) {
  std::optional<Lib> s_lib;
  if (config.contains("lib")) s_lib = Lib{};

  return s_lib;
}

std::optional<Debug> Debug::constructOptionally(const BasicValue &config,
                                                const BasicValue &debug) {
  std::optional<Debug> s_debug;
  if (config.contains("debug")) s_debug = Debug{};

  return s_debug;
}

std::optional<Release> Release::constructOptionally(const BasicValue &config,
...
std::optional<Config> Config::load(const std::string &path) {
  ...
  if (stat(path.c_str(), &info) != 0) return s_config;

  auto config = toml::parse(path);

  s_config->package =
      Package(config, toml::find<BasicValue>(config, "package"));
  s_config->lib =
      Lib::constructOptionally(config, toml::find<BasicValue>(config, "lib"));
  s_config->debug = Debug::constructOptionally(
      config, toml::find<BasicValue>(config, "debug"));
  s_config->release = Release::constructOptionally(
      config, toml::find<BasicValue>(config, "release"));
  s_config->dependencies = Dependencies::constructOptionally(
      config, toml::find<BasicValue>(config, "dependencies"));
